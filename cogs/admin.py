...
class AdminCog(commands.Cog, name="Admin"):
    ...
    async def srblive(self, interaction: discord.Interaction, message: str):
        ...
        if missing_roles:
            logger.warning(f"{interaction.user} sent live notification with missing roles: {missing_roles}")
        else:
            logger.info(f"{interaction.user} sent live notification: {message}")

...
class MidiHelper:
    ...
    def enable(self):
        ...
        try:
            ...
            info(f"Available MIDI output ports: {ports}")
            port_to_use = None
            ...
            self.enabled = True
            self.error = None
            info(f"MIDI output enabled on port: {port_to_use}")
            return True
    ...
    def disable(self):
        with self.lock:
            if self.port:
                try:
                    self.port.close()
                    info("MIDI output port closed.")
                except Exception as e:
                    warning(f"Error closing MIDI port: {e}")
            self.port = None
            self.enabled = False
    ...
    def send_song_change(self):
        ...
        try:
            ...
            info(f"MIDI song change Note On sent (C4, vel={velocity})")

    ...
    if not os.path.exists(settings_path):
        if os.path.exists(example_path):
            try:
                ...
                logger.info(f"Created new settings.json from example at {settings_path}")
            except Exception as e:
                logger.error(f"Failed to copy settings_example.json: {e}")
                tk.Tk().withdraw()
                messagebox.showerror("Error", f"Could not create settings.json: {e}")
                sys.exit(1)
        else:
            logger.error(f"settings_example.json not found in data directory: {example_path}")
            tk.Tk().withdraw()
...
def main():
    ...
    logger.info("🚀 TraCord DJ Launcher")
    logger.info("=" * 40)
